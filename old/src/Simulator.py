...
from HybridSnapReconstructor import HybridSnapReconstructor
...
def snapLimitReconstructData(packetData, logDir, fileEnding, samplingInterval, 
						     interpolationType, snapLimitType, reconThreshold, snapLimit):
	...
	snapData = snapLimitReconstructor.getReconstructedSignal(packetData, samplingInterval,
															 ...
															 reconThreshold, 500, snapLimit)
	exportData(logDir + "SnapLimitData_" + fileEnding + ".txt", snapData)
	
	return [snapData]
	
def hydridSnapReconstructData(packetData, logDir, fileEnding, samplingInterval, 
							  interpolationType, snapLimitType, reconThreshold, snapLimit):
	hybridSnapReconstructor = HybridSnapReconstructor()
	snapData = hybridSnapReconstructor.getReconstructedSignal(packetData, samplingInterval,
	 														  interpolationType, snapLimitType, 
															  reconThreshold, 500, snapLimit)
	exportData(logDir + "HybridSnapData_" + fileEnding + ".txt", snapData)
